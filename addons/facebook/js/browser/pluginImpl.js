...
sendHandlers.postScore = function(opts, cb){
	facebookApp.postScore(opts, function (response) {
		cb && cb(null, {
			response: response,
			canceled: !response
		});
...
sendHandlers.viewScores = function(query, cb){
	facebookApp.viewScores(query, function(response) {
	cb && cb({result: response});
		//logger.log(response);
	});
